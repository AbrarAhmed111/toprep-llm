...
EXTRACTION_SYSTEM_PROMPT = (
    ...
    "If the text is ordinary prose, do NOT turn every paragraph or sentence "
    "into a topic -- prefer a smaller set of genuinely useful learning topics "
    "over exhaustive extraction.\n\n"
    "If the text is already a structured list or outline instead (headings "
    "with bullet points underneath, e.g. a syllabus, curriculum, or roadmap), "
    "it is a human-curated topic list -- extract it FAITHFULLY and "
    "EXHAUSTIVELY: every bullet/sub-item is its own topic, not just the "
    "heading above it. A heading with no bullets under it is itself a topic.\n\n"
    ...
    "this includes meta-commentary about the document itself (study advice, "
    "disclaimers, a suggested learning order, \"how to use this guide\"), "
    "which is not a topic even when it appears under its own heading. "
...
NORMALIZATION_SYSTEM_PROMPT = (
    ...
    "Find ONLY the groups of two or more indices that refer to the exact same "
    "concept, and choose the clearest, most natural learner-facing name for "
    "each group. Do NOT merge topics that are merely related but conceptually "
    'distinct -- for example "React State" and "Redux" are related but must '
    "stay separate groups.\n\n"
    "Do NOT list a topic that has no duplicate -- omit it entirely rather than "
    "reporting it as a group of one; every index you don't mention is kept "
    "as-is under its original name. Most documents have few or no duplicates, "
    "so a short or empty list is normal and expected -- keep your answer as "
    "short as the actual duplicates require.\n\n"
    "Respond with ONLY a JSON object of this exact shape:\n"
    '{"groups": [{"name": "<canonical name>", "indices": [<index>, <index>, ...]}, ...]}\n\n'
